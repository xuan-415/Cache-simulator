...
void generate_output(Results cache_results, Desc cache_description, char *output_name){
    FILE *ptr_file_output;
    ptr_file_output=fopen(output_name, "wb");
    float hit_rate = (double)cache_results.write_hits / (double)cache_results.acess_count;
    float miss_rate = (double)cache_results.write_misses / (double)cache_results.acess_count;
    if (!ptr_file_output) {
        printf("\nThe file of output can't be writed\n\n");
    }
    else {
        fprintf(ptr_file_output, "number of set: %d\n", cache_results.number_of_sets);
        double tmp = log2((double)cache_description.line_size);
        double tmp1 = log2((double)cache_results.number_of_sets);
        fprintf(ptr_file_output, "num_block_offset_bit: %.f\n", tmp);
        fprintf(ptr_file_output, "num__index_bit: %.f\n", tmp1);
        fprintf(ptr_file_output, "num_tag_bit: %.f\n", cache_description.len_mem_addr - tmp - tmp1);
        fprintf(ptr_file_output, "num_total_access: %d\n", cache_results.acess_count);
        fprintf(ptr_file_output, "num_hit: %d\n", cache_results.write_hits);
        fprintf(ptr_file_output, "hit_rate: %f\n", hit_rate);
        fprintf(ptr_file_output, "miss_rate: %f\n", miss_rate);
    }
...
int main(int argc, char **argv)               // Files are passed by a parameter
{
    ...
    if (!ptr_file_specs_cache) {
        ...
        fscanf(ptr_file_specs_cache, "-capacity                 %d\n", &cache_description.capacity);
        fscanf(ptr_file_specs_cache, "-block_size               %d\n", &cache_description.line_size);
        fscanf(ptr_file_specs_cache, "-#_of_way                 %d\n", &cache_description.associativity);
        fscanf(ptr_file_specs_cache, "-length_mem_addr          %d\n", &cache_description.len_mem_addr);
        fscanf(ptr_file_specs_cache, "-way_replacement          %s\n", cache_description.replacement_policy);
        printf("%d\n", cache_description.capacity);
        printf("%d\n", cache_description.line_size);

        cache_description.number_of_lines = cache_description.capacity / cache_description.line_size;
    }
    ...
    cache_results.number_of_sets = cache_description.number_of_lines / cache_description.associativity;
    number_of_sets = cache_results.number_of_sets;
    ...
    if (!ptr_file_input) {
        ...
        while (fscanf(ptr_file_input, "0x%x\n%x\n", &a, &b) != EOF){
    ...
    /****************************** Output ************************************/
    generate_output(cache_results, cache_description, output_name);
    /**************************************************************************/
